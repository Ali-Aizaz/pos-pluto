...
function UserDetails({
  ...
  return (
    <section className="flex flex-col items-center border border-black/10 rounded-xl p-3">
      <div className="ml-5 flex max-h-100 w-76 flex-col items-center font-medium overflow-y-auto scrollbar-track-transparent">
        {customerName && (
          <div className="bg-theme-light-black/40 w-full p-2">
            <h1 className="text-xl text-black ">{customerName}</h1>
            <h2 className="capitalize text-theme-purple text-xl">Name</h2>
          </div>
        )}
        {customerPhone && (
          <div className="bg-theme-light-black/20 w-full p-2">
            <h1 className="text-xl text-black ">{customerPhone}</h1>
            <h2 className="capitalize text-theme-purple text-xl">
              Phone number
            </h2>
          </div>
        )}
        {product &&
          Object.keys(product.details).map((key) => (
            <div key={key} className="bg-theme-purple/40 w-full p-2">
              <h2 className="capitalize text-theme-purple text-xl">{key}</h2>
              <h1 className="text-xl font-medium text-black/60">
                {product.details[key]}
              </h1>
            </div>
          ))}
        {warranty && (
          <div className="bg-theme-light-black/20 w-full p-2">
            ...
            <h2 className="capitalize text-theme-purple text-xl">Warranty</h2>
          </div>
        )}
        {price && (
